...
def train_conv_net(datasets,
    ...
    while (epoch < n_epochs):
        ...
        print('epoch: %i, training time: %.9f secs, train perf: %.9f %%, val perf: %.9f %%' % (epoch, time.time()-start_time, train_perf * 100., val_perf*100.))
...
def train(datafile, cv, non_static=True, rand_wv=True):
    ...
    results = []
    for i in range(cv):
        datasets = make_idx_data_cv(revs, word_idx_map, i, max_l=max_l, k=dim, filter_h=7)
        perf = train_conv_net(
            datasets,
            wv,
            img_w=dim,
