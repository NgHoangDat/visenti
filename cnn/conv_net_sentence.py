...
def train_conv_net(
    ...
    for i in range(len(filter_hs)):
        ...
        conv_layer = LeNetConvPoolLayer(
            rng,
            input=layer0_input, image_shape=(batch_size, 1, img_h, img_w),
    ...
    while epoch < n_epochs:
        ...
        report.write('%i\t%.9f\t%.9f\t%.9f\n' % (epoch, time.time() - start_time, train_perf * 100., val_perf * 100.))
        print("epoch: {0} - time: {1} - train: {2} - val: {3}".format(epoch, time.time() - start_time, train_perf * 100., val_perf * 100.))
...
def train(
    ...
    with open(report_file, 'a') as report:
        ...
        report.write("\nNon static: {0}\n".format(non_static))
        report.write("Word embedding: {0}\n".format(we))
        report.write("Batch size: {0}\n".format(batch_size))
        report.write("Shuffer batch: {0}\n".format(shuffle_batch))
        report.write("Filter hs: {0}\n".format(filter_hs))
        report.write("Hidden units: {0}\n".format(hidden_units))
        report.write("Dim: {0}\n".format(dim))
